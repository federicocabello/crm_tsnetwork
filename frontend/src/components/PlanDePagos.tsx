...
type Cuota = {
  monto: number;       // monto base de la cuota (sin interés)
  interes: number;     // % de interés de esa cuota
...
export default function PlanDePagos({ idCliente, idCita, onGuardado }: Props) {
  ...
  const montoPorCuota = numCuotas > 0 && montoTotal > 0
    ? Math.round((montoTotal / numCuotas) * 100) / 100
    : 0;

  // Regenerar cuotas al cambiar número de cuotas, monto total o interés global
  useEffect(() => {
  ...
  }, [numCuotas, montoTotal, interesGlobal]);
  ...
  // Total real = suma de (monto + monto * interes/100) por cada cuota
  ...
  const diferencia = Math.round((montoTotal - totalBase) * 100) / 100;
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      setErrorGuardar("Error de conexión con el servidor.");
    }
    setLoading(false);
  };
    ...
    loading ? <Loading /> : (
      <div className="w-full max-w-2xl mx-auto space-y-4">
        ...
        <div className="rounded-2xl border border-white/10 bg-zinc-900 shadow-lg shadow-black/20 p-5 space-y-4">
          <p className="text-xs font-bold tracking-wide text-white/40 uppercase">Configuración</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            {/* Interés global (se aplica a todas las cuotas, editable por cuota) */}
            <div className="space-y-1">
              <label className="text-xs text-white/50 flex items-center gap-1">
          ...
          {/* Resumen */}
          {montoTotal > 0 && (
            <div className="flex flex-wrap gap-3 pt-1" hidden>
              <div className="flex items-center gap-2 rounded-xl bg-black/20 border border-white/10 px-3 py-1.5 text-xs">
                <span className="text-white/40">Total con interés:</span>
                <span className="text-orange-400 font-bold">
                  ${totalReal.toFixed(2)}
                </span>
              </div>
              <div className={`flex items-center gap-2 rounded-xl border px-3 py-1.5 text-xs transition-colors ${Math.abs(diferencia) < 0.01
                ? "bg-green-500/10 border-green-500/30"
                : "bg-red-500/10 border-red-500/30"
                }`}>
                <span className="text-white/40">Diferencia base:</span>
                <span className={`font-bold ${Math.abs(diferencia) < 0.01 ? "text-green-400" : "text-red-400"}`}>
            ...
            <div className="divide-y divide-white/5 max-h-72 overflow-y-auto">
              {cuotas.map((cuota, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="grid grid-cols-12 gap-2 items-center px-4 py-2.5 hover:bg-white/[0.03] transition-colors"
                  >
                    {/* Número */}
            ...
            <div className="flex items-center justify-between gap-2 px-4 py-3 border-t border-white/10 bg-black/20">
              <div className="col-span-5">
                ...
                <span className={`ml-2 text-sm font-bold ${Math.abs(diferencia) < 0.01 ? "text-green-400" : "text-orange-400"}`}>
                  <FormatearNumero numero={totalReal} />
                </span>
              </div>
              ...
              {cuotas.length > 0 && (
                <div className="flex justify-end gap-3">
                  <button
                    ...
                    className={`flex items-center gap-2 rounded-xl px-5 py-2 text-sm font-bold transition-all ${guardado
        ...
        {cuotas.length === 0 && (
          <div className="rounded-2xl border border-dashed border-white/10 bg-zinc-900/50 p-8 text-center">
            ...
            <p className="text-sm text-white/30">Ingresá un monto y configurá las cuotas para comenzar</p>
          </div>
        )}
      </div>
