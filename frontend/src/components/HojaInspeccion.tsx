...
import { X, Plus, Save, Trash2, PackageSearch, FileText } from "lucide-react";
...
export default function HojaInspeccion({ idCita, idHoja, onClose, onSaved }: HojaInspeccionProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const resInspeccion = await fetch(`${API_URL}/api/inspeccion/${idCita}`);
        if (resInspeccion.ok) {
      ...
      if (!inspeccionTieneItems && idHoja) {
        try {
          ...
          if (resCotizacion.ok) {
            ...
            if (dataCotizacion.productos && dataCotizacion.productos.length > 0) {
              // Mapear artículos de cotización al formato de inspección
              const itemsDeCotizacion: InspeccionItem[] = dataCotizacion.productos.map((p: any) => ({
                producto_id: p.id,
            ...
            console.error("Error al obtener cotización, status:", resCotizacion.status);
          }
        } catch (error) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="bg-zinc-900 border border-white/10 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[92dvh] sm:max-h-[88vh]">
        ...
        <div className="bg-zinc-800 px-4 py-3 border-b border-white/10 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-bold text-white leading-tight">Hoja de Inspección</h2>
              <p className="text-xs text-white/50">Materiales para la instalación</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1 flex flex-col">
          {loading ? (
            ...
            <>
              {/* Add Item Form */}
              <div className="p-4 border-b border-white/10 bg-zinc-950/40 flex flex-col gap-3">
                <p className="text-xs font-bold text-white/40 uppercase tracking-wider">Agregar material</p>

                {/* Producto selector */}
                <div>
                  <label className="block text-xs font-semibold text-white/60 mb-1">Producto</label>
                  <select
                    value={selectedProducto}
                    onChange={(e) => setSelectedProducto(e.target.value ? Number(e.target.value) : "")}
                ...
                <div className="flex gap-2">
                  <div className="w-24 shrink-0">
                    <label className="block text-xs font-semibold text-white/60 mb-1">Cant.</label>
                    <input
                      type="number"
                      min="1"
                      value={cantidad}
                  ...
                  <div className="flex-1">
                    <label className="block text-xs font-semibold text-white/60 mb-1">Detalle (opcional)</label>
                    <input
                      type="text"
                      value={detalle}
                ...
                {items.length === 0 ? (
                  ...
                  <>
                    ...
                    {items.map((item, index) => (
                      <div
                        key={index}
                        className="flex items-start gap-3 bg-zinc-800/60 border border-white/8 rounded-xl p-3">
                        {/* Cantidad badge */}
                        <div className="shrink-0 bg-orange-500/15 border border-orange-500/30 rounded-lg px-2 py-1 text-center min-w-[2.5rem]">
                          <span className="text-orange-300 font-bold text-sm">{item.cantidad}</span>
                        </div>
