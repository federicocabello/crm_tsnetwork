...
import {
  ...
  export default function NuevoRegistro() {
    ...
    const [opcionCamaras, setOpcionCamaras] = useState<"tiene" | "desdecero" | "tieneclientenuevo"  | "tieneclienteexistente" | null>(null);
  ...
  const canEdit = ["moderador", "administrador", "superadmin"].includes(user?.rol ?? "invitado");
  ...
  const handleChangeDescripcion = (e: React.ChangeEvent<HTMLTextAreaElement>, id: number) => {
  ...
  const handleChangeTitulo = (e: React.ChangeEvent<HTMLInputElement>, id: number) => {
    const updatedSpeechItems = speechItems.map((item) => 
      ...
      <div className="cuadro w-1/4 shrink-0 cuadro min-h-0 overflow-y-auto">
        <div className="flex justify-between items-center">
          <h2 className="text-sm font-extrabold tracking-tight flex items-center gap-1">
            <NotebookTabs className="w-4 h-4" /><span>Nuevo registro</span>
          </h2>
        ...
        {selectedOption == "camaras" && (
          <div className="flex my-3 gap-2">
            <button className={`boton w-full p-1! text-xs! border flex gap-2 items-center justify-center ${
        ...
        {opcionCamaras == "desdecero" && (
          <div className="flex flex-col gap-3">
            ...
            <div>
              ...
              {formRegistro.nombre && formRegistro.direccion && formRegistro.telefono && formRegistro.fecha && hora && formDataCamaras.lugar && formDataCamaras.audio && formDataCamaras.area &&formDataCamaras.monitor && formDataCamaras.atico && formDataCamaras.cantidad && formDataCamaras.estructura && (
  ...
  {selectedOption && (
          ...
          <div className="flex-1 min-h-0 overflow-y-auto pr-2">
            <DndContext collisionDetection={closestCenter} onDragEnd={onDragEnd}>
              <SortableContext items={speechItems.map((s) => s.id)} strategy={verticalListSortingStrategy}>
                <div className="grid grid-cols-2 gap-2">
                  {speechItems.map((item) => (
                    <SortableSpeechCard key={item.id} item={item} isEditing={isEditing}>
                      <input className="speech-titulo" value={item.titulo} disabled={!isEditing} onChange={(e) => {handleChangeTitulo(e, item.id)}} />
                      {!isEditing ? (
                        <div
                          className="speech-descripcion whitespace-wrap leading-relaxed"
                          dangerouslySetInnerHTML={{ __html: formatBoldStars(item.descripcion) }}
                        />
                      ) : (
                        <textarea
                          ref={(el) => {
                      ...
                      <span hidden={!isEditing} className="transition-all mt-auto">
                        <Trash2 className="w-6 h-6 text-white/30 hover:text-red-500 drop-shadow-xs drop-shadow-white hover:drop-shadow-red-500 cursor-pointer transition-all mt-2 ml-auto" onClick={() => removeItem(item.id)} />
                      </span>
                    </SortableSpeechCard>
                  ))}

                  {(user.rol == "moderador" || user.rol == "administrador" || user.rol == "superadmin") && (isEditing && (
                      <div className="shadow-lg shadow-black/20 bg-zinc-900 rounded-2xl border border-white/10 hover:border-orange-500 p-4 flex items-center justify-center gap-1 cursor-pointer inset-shadow-sm hover:inset-shadow-orange-500 transition-all font-bold hover:text-orange-500" onClick={() => agregarSpeech()}>
        ...
        <div className="fixed bottom-6 right-6 flex flex-col gap-3 z-50">
          ...
          <button
            ...
            className={[
              ...
              isEditing ? "bg-orange-500 text-white hover:bg-orange-600" : "bg-white/10 text-white hover:bg-white/15",
            ].join(" ")}
          ...
          {isEditing && (
            <button
              ...
              className={[
                "rounded-2xl px-4 py-3 shadow-xl border border-white/10 backdrop-blur",
                "flex items-center gap-2 font-extrabold cursor-pointer",
                saving ? "bg-zinc-700 text-white/70 cursor-not-allowed" : "bg-green-600 text-white hover:bg-green-700",
              ].join(" ")}
