...
import { ClipboardList, Drill, Wrench, CircleDollarSign, Files, Home, Phone, Loader2, Save } from 'lucide-react';
...
export default function FormularioCamarasExistente({ tipoRegistro = "camaras" }: FormularioCamarasExistenteProps) {
  ...
  cotizacionInicial={presupuesto}
        ...
        <button
          type="button"
          disabled={guardando || !clienteSeleccionado || !formRegistro.fecha || !hora || ((esInternet && !estadoInternet) || (!esInternet && !opcionTipoInstalacion))}
          onClick={submitFormularioCamarasTiene}
          className="inline-flex items-center justify-center gap-2 rounded-xl bg-orange-600 px-4 py-3 text-sm font-black text-white hover:bg-orange-700 cursor-pointer disabled:cursor-not-allowed disabled:opacity-40 shadow-lg shadow-orange-600/20 transition w-full mt-3">
          {guardando ? (
