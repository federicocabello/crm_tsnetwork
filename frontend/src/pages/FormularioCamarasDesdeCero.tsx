...
export default function FormularioCamarasDesdeCero({ onChange }: FormularioProps) {
  const [lugar, setLugar] = useState<"casa" | "negocio" | "traila" | "foodtruck" | "apartamento" | null>(null);
  const [cantidad, setCantidad] = useState("");
  const [audio, setAudio] = useState<"si" | "no" | null>(null);
  const [area, setArea] = useState<"interior" | "exterior" | null>(null);
  const [atico, setAtico] = useState<"espacioso" | "espuma" | "no tiene" | null>(null);
  const [monitor, setMonitor] = useState<"si" | "no" | null>(null)
  const [estructura, setEstructura] = useState<"movil" | "standard" | "casona" | null>(null);
  ...
  }, [lugar, cantidad, audio, area, atico, monitor, estructura, presupuesto]);
  ...
  return (
    <div className="flex flex-col gap-3 w-full">
      ...
      {lugar == "foodtruck" && (
        <div className="text-xs italic transition-all mt-1 text-white/60">ACLARACIÓN: preguntar si cuenta con internet y monitor. <strong className="text-red-500">Sin internet no
podemos proceder con la instalación.</strong></div>
        )}
        {lugar == "apartamento" && (
        <div className="text-xs italic transition-all mt-1 text-white/60">ACLARACIÓN: es necesario obtener el permiso del arrendador <strong className="text-red-500">para perforar el techo o las paredes</strong> antes de realizar cualquier instalación.</div>
        )}
    </div>
    ...
    <div>
      ...
      <div className="flex flex-col gap-1">
        ...
        {atico && (
        <div className="text-xs italic transition-all mt-1 text-white/60">ACLARACIÓN: con <strong className="text-red-500">insulación normal</strong> (fibra/lana), el cable puede pasarse sin romper paredes, lo que facilita una instalación rápida y estándar. Con <strong className="text-red-500">insulación de espuma</strong>, bloquea el paso del cable, haciendo la instalación más compleja y requiriendo perforaciones o canaletas externas.</div>
        )}
    </div>
...
<div>
    ...
    {estructura && (
        <div className="text-xs italic transition-all mt-1 text-white/60">ACLARACIÓN: en caso de que la casa sea nueva y <strong className="text-red-500">no aparezca en el mapa</strong>, pedir fotos de la misma <strong className="text-red-500">completa de frente, atrás y costados.</strong></div>
    )}
    </div>
    </div>
  );
